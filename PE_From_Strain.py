...
parser.add_argument("--detector", type=str, required=True, help="GW detector used to recover system.")
...
# Set the duration and sampling frequency of the data segment we are injecting into
...
if args.source_type in ["BNS", "NSBH"]:
...
if args.detector == "LIGO":
    ...
    #ifos[0].power_spectral_density = bilby.gw.detector.PowerSpectralDensity(asd_file = "/home/giarratana/TaylorF2_SPA_Modification/CE_wb_asd.txt")
    #ifos[1].power_spectral_density = bilby.gw.detector.PowerSpectralDensity(asd_file = "/home/giarratana/TaylorF2_SPA_Modification/CE_wb_asd.txt")

    ifos.set_strain_data_from_power_spectral_densities(
            sampling_frequency=sampling_frequency,
            duration=duration,
            start_time=injection_parameters["geocent_time"] - args.begin, 
            )
elif args.detector == "CE":
    ifos = bilby.gw.detector.InterferometerList(["CE"])
    ifos[0].power_spectral_density = bilby.gw.detector.PowerSpectralDensity(psd_file = "/home/giarratana/TaylorF2_SPA_Modification/CE_wb_psd.txt")
    ifos.set_strain_data_from_power_spectral_densities(
            sampling_frequency=sampling_frequency,
            duration=duration,
            start_time=injection_parameters["geocent_time"] - args.begin,
            )
else: raise ValueError(f"Unknown detectorr type: {detector}")
...
if source == 'BBH':
    ...
    #priors.pop('total_mass')
    #priors.pop('symmetric_mass_ratio')
    #priors['mass_1'] = bilby.core.prior.Uniform(name='mass_1', minimum=m1*0.5, maximum=m1*1.5)
    #priors['mass_2'] = bilby.core.prior.Uniform(name='mass_2', minimum=m2*0.5, maximum=m2*1.5)
    
    priors['chi_1'] = bilby.core.prior.Uniform(0, 0.1, 'chi_1')
    ...
    #priors.pop('total_mass')
    #priors.pop('symmetric_mass_ratio')
    #priors['mass_1'] = bilby.core.prior.Uniform(name='mass_1', minimum=5, maximum=20)
    #priors['mass_2'] = bilby.core.prior.Uniform(name='mass_2', minimum=1, maximum=3)
...
conversion_function_map = {
        ...
        "NSBH": generate_all_bns_parameters_custom,
        }
